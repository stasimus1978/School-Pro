...
export default function SearchBar({ data, onSearch, setIsSearch }: { data: any[]; onSearch: any; setIsSearch: any }) {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredData = data.filter((item: any) =>
      Object.values(item).some(
        (value: any) => value && value.toString().toLowerCase().includes(e.target.value.toLowerCase())
      )
    );
    setIsSearch(true);
